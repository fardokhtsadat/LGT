...
result = main(df) # main(df) calls remove_duplicate_accession()
empty_df = pd.DataFrame()
...
result1 = get_hitproportion_meaneval(empty_df)

#--------------run get taxid
with Pool(20) as p:
    res = p.map(get_taxid, edit_acc(list(result1['sseqid'])))
    Glob = {} # to join the results from the parallel run
    for i in res:
        Glob.update(i)

result1['taxid'] = np.nan #add a new col
for i in range(len(result1['sseqid'])):
    edited = result1['sseqid'][i].split('.', 1)[0]
    result1.loc[result1['sseqid'] == result1['sseqid'][i], 'taxid'] = Glob[edited]

unique_taxids = list(set(result1['taxid']))
with open('all_taxids.txt', 'w') as f:
    for item in unique_taxids:
        f.write("%s\n" % item)

#--------------run get taxonomic rank
...
for i in files:
    result = get_taxonomic_rank(i)
    taxid_full_rank.update(result)
...
for i in taxid_rank:
    result1.loc[result1['taxid'] == int(i), ['rank', 'name']] = [taxid_rank[i][0], taxid_rank[i][1]]

#phylum_only is a list created from ncbi/taxonomoy_browser/bacteria. this list contains the phylumns under the category bacteria
...
joined_df = result1.sort_values(['occurrence', 'mean_eval'], ascending=[False, True])
top_df = result1.groupby('name').head(3) #we get the top 3
top_df = top_df.dropna() #remove nan values

random = pd.concat([result1, top_df]).drop_duplicates(keep=False)

#the following gets the random elements for each group:
